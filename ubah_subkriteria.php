...
<body class="font-['Inter'] bg-gray-100 flex min-h-screen justify-center items-center">
    ...
    <script type="text/javascript">
        ...
        function resetSubcriteria() {
            // Dapatkan semua elemen input subkriteria
            const subcriteriaInputs = document.querySelectorAll('[id^="subkriteria_"]');
            // Dapatkan semua elemen select nilai subkriteria
            const subcriteriaSelects = document.querySelectorAll('[id^="nilai_subkriteria_"]');

            // Lakukan reset nilai untuk setiap elemen input subkriteria
            subcriteriaInputs.forEach(input => {
                input.value = '';
            });

            // Lakukan reset nilai untuk setiap elemen select nilai subkriteria
            subcriteriaSelects.forEach(select => {
                const options = select.querySelectorAll('option');
                options.forEach(option => {
                    option.disabled = false; // Aktifkan kembali semua opsi yang dinonaktifkan
                    option.selected = option.defaultSelected; // Setel kembali nilai opsi ke nilai default
                });
            });
        }
    </script>
